...
import json
import numpy as np
from Exp3_DataSet import trainset
    ...
    if not os.path.exists('data/location_vector.txt'):
        for i in range(len(trainset)):
            location = get_location(trainset[i]['head'],
                                    trainset[i]['tail'], 
                                    trainset[i]['text'])
            location.append(relation2id[trainset[i]['relation']])
